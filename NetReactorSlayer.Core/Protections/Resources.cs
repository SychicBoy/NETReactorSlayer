...
namespace NETReactorSlayer.Core.Protections
{
    class Resources
    {
        static readonly int[] pktIndexes = new int[]
          {
            1,
            0,
            3,
        ...
        public static void Execute()
        {
                                    ...
                                    try
                                    {
                                        if (instruction.OpCode.Equals(OpCodes.Call))
                                        {
                                            if (instruction.Operand is MethodDef decryptorMethod && decryptorMethod.DeclaringType.Equals(type) && !decryptorMethod.HasReturnType)
                                            {
                                                ...
                                                {
                                                    if ((encryptedResource = (DotNetUtils.GetResource(Context.Module, s) as EmbeddedResource)) != null)
                                                    {
            ...
            try
            {
                ...
                {
                    Remover.MethodsToPatch.Add(m);
                }
                Remover.ResourceToRemove.Add(encryptedResource);
                Logger.Info("Assembly resources decrypted");
            }
            catch (Exception ex)
            {
