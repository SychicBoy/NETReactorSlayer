...
internal class StrongName : IStage
{
    public void Execute()
    {
        var count = 0L;
        ...
        foreach (var method in type.Methods.Where(x => x.HasBody && x.Body.HasInstructions))
            try
            {
                ...
                if (block?.FallThrough == null || block.Targets is not {Count: 1})
                    continue;
                count += 1L;
                ...
                if (block.FallThrough.Instructions.Count <= 1 && block.FallThrough.LastInstr.OpCode.Code == Code.Nop &&
                    ...
                    {
                        block.FallThrough.Parent.RemoveGuaranteedDeadBlock(block.FallThrough);
                        block.FallThrough.FallThrough.Parent.RemoveGuaranteedDeadBlock(block.FallThrough.FallThrough);
                    }

                blocks.GetCode(out var allInstructions, out var allExceptionHandlers);
                DotNetUtils.RestoreBody(method, allInstructions, allExceptionHandlers);
            } catch { }
        if (count > 0L) Logger.Done($"Strong name removal protection removed from {(int)count} methods.");
        else Logger.Warn("Couldn't find any strong name removal protection.");
    }

    private static Block GetBlock(Blocks blocks, IMethod methodDef) => (from block in blocks.MethodBlocks.GetAllBlocks()
        ...
        where instructions[i + 7].OpCode.Code is Code.Call or Code.Callvirt &&
        ...
        where instructions[i + 9].OpCode.Code is Code.Call or Code.Callvirt &&
    ...
    private static MethodDef Find() => (from type in Context.Module.GetTypes()
        from method in type.Methods
        where method.IsStatic && method.Body != null
        let sig = method.MethodSig
        where sig != null && sig.Params.Count == 2
        where sig.RetType is {ElementType: ElementType.Object or ElementType.String}
