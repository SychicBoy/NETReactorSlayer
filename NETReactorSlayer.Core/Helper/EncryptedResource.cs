...
internal class EncryptedResource : IDisposable
{
    ...
    public static bool IsKnownDecrypter(MethodDef method, IList<string> additionalTypes, bool checkResource)
    {
        ...
        if (checkResource)
        {
            if (method is not { HasBody: true, Body.HasInstructions: true })
                return false;
        ...
        if (method is not { IsStatic: true, HasBody: true })
            return false;
    ...
    {
        if (DecrypterMethod is not { IsStatic: true, HasBody: true })
            return null;
